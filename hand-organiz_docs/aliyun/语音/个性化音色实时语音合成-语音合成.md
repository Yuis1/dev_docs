---
url: https://help.aliyun.com/document_detail/2842586.html?mode=pure
title: "个性化音色实时语音合成-语音合成-大模型服务平台百炼-阿里云"
captured_at: "2026-01-25T01:05:00.303Z"
---


# 个性化音色实时语音合成-语音合成-大模型服务平台百炼-阿里云
...
- **支持的模型：****CosyVoice：**cosyvoice-v3-plus、cosyvoice-v3-flash、cosyvoice-v2、cosyvoice-v1
...
| **CosyVoice**将合成音频保存为文件将LLM生成的文本实时转成语音并通过扬声器播放
 **
 **
 PythonJava
 **
...
```
 ...
 以下代码展示通过本地设备播放通义千问大语言模型（qwen-turbo）实时返回的文本内容。PythonJava
 **
 **
...
public class Main {
 ...
 // Create a subclass inheriting from ResultCallback
 ...
 Flowable result = gen.streamCall(genParam);
...
``` |
| --- |
| **Sambert**将合成音频保存为文件将合成的音频通过扬声器播放
 **
 **
 PythonJava
 **
...
```
 ...
 合成语音后，通过本地设备播放实时返回的音频内容。运行Python示例前，需要通过pip安装第三方音频播放库。PythonJava
 **
 **
 *Python*
 **
...
public class Main {
 ...
 // 创建一个继承自ResultCallback的子类来实现回调接口
 class ReactCallback extends ResultCallback {
 private PlaybackRunnable playbackRunnable = null;
